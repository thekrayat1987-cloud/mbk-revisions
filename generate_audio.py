#!/usr/bin/env python3
"""Generate MP3 audio files for all dialogues using gTTS."""
...
dialogues = {
    ...
    'd5': [
        ...
        "Brigitte : Non, notre maison est proche. Elle est à 5 km de Paris.",
...
for did, lines in dialogues.items():
    path = f'audio/{did}.mp3'
    print(f'Generating {path}...', end=' ', flush=True)
    full_text = '  '.join(lines)
    tts = gTTS(full_text, lang='fr', slow=False)
    tts.save(path)
    print('OK')

print('\nDone! All audio files generated in ./audio/')
